(function (window) {
  ...
  var Dictate = function (cfg) {
    ...
    this.init = function () {
      ...
      config.onEvent(MSG_WAITING_MICROPHONE, "请求麦克风使用授权 ...");
      try {
        ...
        config.onError(ERR_CLIENT, "初始化浏览器音频处理功能(Web Audio)失败: " + e);
      }
      if (navigator.getUserMedia) {
        ...
        navigator.getUserMedia(audioSourceConstraints, startUserMedia, function (e) {
          config.onError(ERR_CLIENT, "此浏览器中不支持麦克风录音: " + e);
        });
      } else {
    ...
    this.startListening = function () {
      if (!recorder) {
        config.onError(ERR_AUDIO, "无录音设备!");
        return false;
      }
      if (ws) {
        cancel();
      ...
      try {
        ...
        config.onError(ERR_CLIENT, "此浏览器不支持Web Socket,无法向服务发送语音数据!");
        return false;
      }
      return true;
    }
    ...
    this.stopListening = function () {
      ...
      if (recorder) {
        ...
        config.onError(ERR_AUDIO, "无法启动录音设备!");
      }
    }
    ...
    function startUserMedia(stream) {
      ...
      config.onEvent(MSG_INIT_RECORDER, '录音设备初始化成功!');
      ...
      navigator.mediaDevices.enumerateDevices().then(device =>{
        for(var i=0; i<device.length; i++){
          if(device[i].deviceId === window.deviceId){
            window.deviceLable = device[i].label;
            break;
          }
        }
      });
      ...
      if (ws) {
        ...
        if (state == 1) {
          ...
          if (item instanceof Blob) {
            if (item.size > 0) {
              ws.send(item);
              config.onEvent(MSG_SEND, '发送: blob: ' + item.type + ', ' + item.size);
            } else {
              config.onEvent(MSG_SEND_EMPTY, '发送: blob: ' + item.type + ', EMPTY');
            }
            // Otherwise it's the EOS tag (string)
          } else {
            ws.send(item);
            config.onEvent(MSG_SEND_EOS, '发送 标记: ' + item);
          }
          //绘制录音波纹
          drawCircleWave();
        } else {
          config.onError(ERR_NETWORK, 'WebSocket: readyState!=1: ' + state + ": 发送失败");
        }
      } else {
        config.onError(ERR_CLIENT, '无Web socket连接: 发送失败: ' + item);
      }
    }
        ...
        if (data instanceof Object && !(data instanceof Blob)) {
          ...
          try {
            ...
            if (res.status == 0) {
              ...
              config.onError(ERR_SERVER, '服务端错误: ' + res.status + ': ' + getDescription(res.status));
            }
          } catch (e) {
            console.log(data);
          }
